...
$processor = new Processor(
    ...
    clock: $clock,
    types: $endpointProvider->configuredTypes(),   // только то, у чего есть эндпоинты
);
...
`process()` не принимает аргументов; что забирает процессор, решает `types:`
при конструировании. Опустите его — и он заберёт **все** ожидающие сообщения
из хранилища независимо от типа; прочитайте следующий раздел, прежде чем
делать так на outbox, из которого читают другие потребители.
...
маршрутизирует, а нескоупленный `Processor` забирает всё, включая их, и
паблишер подтверждает их с нулём доставок. Ничего не логируется, ни один
алерт не срабатывает.

Ограничьте процессор типами, которыми владеет паблишер. С
`ConfigWebhookEndpointProvider` это один вызов — `configuredTypes()`
перечисляет каждый тип, у которого есть хотя бы один эндпоинт, — а со своим
провайдером это тот же список, который вы и так ему отдаёте:
...
Скоуп доходит до `claim()` хранилища, так что чужое сообщение даже не видно.
Ещё две линии обороны — на случай, когда нельзя положиться на то, что скоуп
не разойдётся с картой эндпоинтов:
...
- URL эндпоинта маскируется перед тем, как попасть в сообщение
  `PublishException`. `Processor` из `yii3-outbox` пишет это сообщение в лог
  как есть, поэтому эндпоинт с токеном в query-строке (`?access_token=…`) или
  в userinfo (`https://user:pass@…`) иначе оказался бы во всех приёмниках
  логов приложения. Схема, хост, порт, путь и ключи query сохраняются —
  строка лога остаётся диагностируемой; значения query, пароль и fragment
  заменяются на `***`. Передавать учётные данные всё равно лучше заголовками
  на уровне `WebhookDispatcher`: маскирование — страховка, а не разрешение.
- Апстримная половина этого сообщения тоже вычищается. Строка сбоя выглядит как
  `<маскированный эндпоинт>: <ошибка апстрима>`, и вторую половину пишет не этот
  пакет — это `getLastError()` доставки или сообщение того, что бросил
  диспетчер. PSR-18 клиент штатно кладёт туда полный request URI
  (`cURL error 7: Failed to connect ... for https://host/e?access_token=…`),
  так что маскирования одного эндпоинта не хватило бы: учётные данные вернулись
  бы в лог. Каждый секрет, который несёт URL эндпоинта, удаляется из этого
  текста везде, где встретится, — в percent-encoded виде и без. Короткое
  значение query (`?page=1`) заставит вычистить и лишнее вокруг: из двух ошибок
  эта дешевле.
